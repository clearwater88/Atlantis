function [boundary,template] = getPatchTransformInds(template,poseCentre)
    % pose is [xCentre,yCentre,angle]

    template = imrotate(template,-180*(poseCentre(3))/pi,'nearest','loose');
    boundary(:,1) = (poseCentre(1:2)-(size(template)-1)/2)';
    boundary(:,2) = (poseCentre(1:2)+(size(template)-1)/2)';
end
